...
views = Blueprint('federation_entity', __name__, url_prefix='')
...
@views.route('/fetch')
def fetch():
    _endpoint = current_app.server.get_endpoint('fetch')
    return service_endpoint(_endpoint)


@views.route('/list')
def list():
    _endpoint = current_app.server.get_endpoint('list')
    return service_endpoint(_endpoint)


@views.route('/resolve')
def resolve():
    _endpoint = current_app.server.get_endpoint('resolve')
    return service_endpoint(_endpoint)


@views.route('/status')
def status():
    _endpoint = current_app.server.get_endpoint('status')
    return service_endpoint(_endpoint)
...
@views.route('/.well-known/openid-federation')
def wkof():
    _fe = current_app.server
    metadata = _fe.get_metadata()
    _ctx = _fe.get_context()
