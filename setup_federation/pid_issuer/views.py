...
def service_endpoint(endpoint):
    ...
    http_info = {
        "headers": {k: v for k, v in request.headers.items(lower=True) if k not in ["cookie", "user-agent"]
},
...
@entity.route('/pid_fed_keys.json')
def wp_fed_keys():
...
def token():
    _endpoint = current_app.server["oauth_authorization_server"].get_endpoint('token')
    return service_endpoint(_endpoint)


@entity.route('/authorization', methods=['GET'])
def authorization():
    _endpoint = current_app.server["oauth_authorization_server"].get_endpoint('authorization')
    return service_endpoint(_endpoint)


@entity.route('/authz', methods=['GET'])
def authz():
    _endpoint = current_app.server["oauth_authorization_server"].get_endpoint('authorization')
    return service_endpoint(_endpoint)


@entity.route('/pushed_authorization', methods=['GET'])
def integrity():
...
def credential():
    _endpoint = current_app.server["openid_credential_issuer"].get_endpoint('credential')
    return service_endpoint(_endpoint)
