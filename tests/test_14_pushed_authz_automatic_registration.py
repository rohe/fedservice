...
class TestAutomatic(object):
    ...
    def test_automatic_registration_pushed_authz(self):
        ...
        _context = self.oc["oauth_client"].context
        ...
        _context.provider_info = _context.server_metadata["oauth_authorization_server"]

        # create the authorization request

        request_args = AuthorizationRequest(response_type="code", state=rndstr(),
                                            client_id=self.oc["federation_entity"].entity_id)
        _auth_service = self.oc["oauth_client"].get_service("authorization")
        # authn_request = _auth_service.get_request_parameters(request_args=request_args)
        ...
        with responses.RequestsMock() as rsps:
            ...
            _pushed_authz_endpoint = self.oas["oauth_authorization_server"].get_endpoint("pushed_authorization")
